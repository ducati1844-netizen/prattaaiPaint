...
const STAGES = {
  new_lead:         "C121:NEW",
  msg1_no_answer:   "C121:UC_A0S2XP",  // 1st silence
  msg2_no_answer:   "C121:UC_6RW4V7",  // 2nd silence
  msg3_no_answer:   "C121:UC_13YDAK",  // 3rd silence
  answer:           "C121:UC_DYSLZO",  // Квалификация
  qualify:          "C121:PREPARATION", // Согласование
  clear_task:       "C121:UC_AMH32V",  // Коммерческое предложение
  commercial_offer: "C121:UC_AMH32V",  // Коммерческое предложение
  invoice:          "C121:PREPAYMENT_INVOI", // Счёт на оплату
  waiting_payment:  "C121:PREPAYMENT_INVOI", // Счёт на оплату
  payed:            "C121:EXECUTING",  // Оплата
  colouring:        "C121:FINAL_INVOICE", // Колеровка
  delivery:         "C121:UC_39GF9W",  // Отгрузка/доставка
  received:         "C121:UC_RMP5KA",  // Reciept
  base_for_messages:"C121:UC_13YDAK",  // 3rd silence → база
  won:              "C121:WON",
  lose:             "C121:LOSE",
...
[SEND_CHECKLIST] — отправить чеклист подготовки поверхности
[NOTIFY_MANAGER] — когда клиент готов платить (invoice)

=== ШПАКЛЁВКА (ОБЯЗАТЕЛЬНЫЙ ВОПРОС) ===
Перед нанесением краски ВСЕГДА нужна шпаклёвка. Обязательно уточни:
"Какая шпаклёвка нанесена? Финишная или базовая?"
Почему важно:
- На необработанную/пористую поверхность краска уйдёт в стену → перерасход
- Нестабильная шпаклёвка → отслоение краски
- Гипсовая шпаклёвка во влажных зонах → набухание под краской
Правильная подготовка = предсказуемый результат.

Если шпаклёвка не нанесена или клиент не знает → предложи чеклист:
"Хотите я пришлю чеклист по подготовке поверхности? Там всё по шагам — займёт 2 минуты."
При согласии ставь тег [SEND_CHECKLIST]
...
Только текст, без тегов [STAGE] и прочего.`;
  ...
  try {
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: "claude-sonnet-4-5", max_tokens: 150, system: getSystem(), messages: msgs })
    });
...
async function askClaude(chatId, userMessage) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-api-key": ANTHROPIC_KEY, "anthropic-version": "2023-06-01" },
    body: JSON.stringify({ model: "claude-sonnet-4-5", max_tokens: 1000, system: getSystem(), messages: histories[chatId] })
  });
  const data = await res.json();
  if (!data.content) {
    console.error("Claude API error in askClaude:", JSON.stringify(data));
...
app.post("/webhook", async (req, res) => {
  ...
  if (text) {
    ...
    if (!clientLang[chatId] || detectedLang !== "en") {
      clientLang[chatId] = detectedLang;
    }
  }
  ...
  try {
    ...
    if (sendColorCatalog) {
      console.log("Sending color catalog:", CATALOGS.colors, "exists:", fs.existsSync(CATALOGS.colors));
      await sendLocalDocument(chatId, CATALOGS.colors, "Каталог цветов Pratta 🎨");
      console.log("Color catalog sent");
    }
    if (sendChecklist) {
      if (CHECKLIST_URL) {
...
app.listen(PORT, () => {
  console.log(`Paint Bot running on port ${PORT}`);
  console.log("CATALOG_DIR:", CATALOG_DIR);
  // Проверяем наличие файлов при старте
  Object.entries(CATALOGS).forEach(([product, langs]) => {
    if (typeof langs === "string") {
      const exists = fs.existsSync(langs);
      console.log(`Catalog ${product}: ${exists ? "OK" : "MISSING"} (${langs})`);
    } else {
      Object.entries(langs).forEach(([lang, p]) => {
        const exists = fs.existsSync(p);
        console.log(`Catalog ${product}/${lang}: ${exists ? "OK" : "MISSING"} (${p})`);
      });
    }
  });
});
